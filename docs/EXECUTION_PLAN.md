...
- [x] «Изучить источники» реализовано как одноразовая кнопка под базовым Web ON
  ответом: bounded in-memory TTL-state без записи вопроса на диск, повторная выдача
  до 10 результатов, не более 4 страниц, evidence budget 2800, до 5 ссылок и общий
  deadline 30 секунд. Текущий slice делает один углублённый подзапрос; второй
  параллельный вариант остаётся допустимым будущим улучшением только после замеров
  качества и расхода месячной search quota.
