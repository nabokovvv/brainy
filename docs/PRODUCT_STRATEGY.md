...
5. Получает короткий ответ; кнопка «Подробнее» просит текущий path раскрыть ответ глубже.
...
| Deep Search | Убрать как отдельный режим | Кнопка «Подробнее» внутри текущего Web ON/OFF path |
...
- smoke/eval минимум 50 мультиязычных вопросов перед beta;
