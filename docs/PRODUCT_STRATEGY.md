...
Отдельная стратегия сайта и автономного publishing pipeline зафиксирована в
[`WEBSITE_CONTENT_PIPELINE_PLAN.md`](WEBSITE_CONTENT_PIPELINE_PLAN.md). Сайт не
использует пользовательские диалоги как источник тем или контента и не запускает
generation jobs внутри Telegram runtime.
...
1. Ротировать только явно настроенные free-quota adapters: Tavily → Brave Search
   API → SerpAPI. Лимиты ведутся по UTC календарному месяцу: 900, 900 и 200
   запросов.
2. Один запрос последовательно пробует providers в этом порядке и обычно расходует
   один credit; следующий provider вызывается только после ошибки, quota exhaustion
   или пустой выдачи предыдущего.
