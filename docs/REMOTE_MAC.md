...
### Live smoke 2026-07-11 (Telegram runtime)

`TELEGRAM_TOKEN` уже в runtime `.env` (права `600`). Живой Bot API smoke через
`@askbrainy_com_bot` выполнен; текст пользователя и ответы в логи не писались.

Итоги и внесённые изменения (все закоммичены в ветке):

1. **Citation rendering** — модель отвечает чистой прозой; приложение прикрепляет
   топ-3 найденных источника как кликабельные MarkdownV2-ссылки с читаемым
   доменом (IDNA/punycode декодируется, `www.` убирается, первая буква заглавная)
   и одним link-preview верхнего источника (лимит Telegram — одно превью на
   сообщение).
2. **Web ON UX** — typing-индикатор на web-маршруте, бейдж `🌐 X.Xs`, и фазовый
   progress-draft (searching → synthesizing → живой стриминг).
3. **Синтез переписан на streaming** — на вход модели идут чистые чанки (без ID и
   URL), ответ стримится токен-по-токену под общим LLM-семафором, `<think>`
   вырезается. Источники = топ-N найденных по rank (retrieved, не per-claim).
   Вывод по-прежнему проходит `sanitize_untrusted_markdown` (чанки — untrusted).
4. **Fast Reply** — рендерит markdown модели верно, включая кликабельные ссылки
   (локальный маршрут без внешней инъекции); web-маршрут ссылки модели по-прежнему
   вырезает.
5. **Политика отказов search исправлена** — транзиентные ошибки (таймаут, сеть,
   429, 5xx, битый payload) больше не помечают провайдера failed и не выключают
   Web ON; только 401/403 (плохой ключ) помечают провайдера на месяц, а global
   disable срабатывает лишь при реальном исчерпании/отказе всех провайдеров.

**error-path (все search API недоступны → `web_unavailable` без local fallback)**
подтверждён органически 2026-07-11 (все три провайдера словили ошибки под старой
политикой → `SearchUnavailableError` → локализованный `web_unavailable`) и покрыт
юнит-тестами. Quota-state после инцидента сброшен вручную (бэкап:
`~/.local/state/brainy/search_quota.json.bak`).

### Остаётся

1. Опционально: явный live-повтор error-path (форсировать `global_disabled` в
   `SEARCH_QUOTA_STATE_PATH`, один web-запрос, подтвердить `web_unavailable`,
   восстановить state). По сути уже подтверждён.
2. UX #5: toggle `⚡ Web: ВКЛ/ВЫКЛ` висит вверху чата — перенести в `/settings`
   (отдельный редизайн, не начат).
3. Ключи Tavily/Brave/SerpAPI перевыпуск владельцем не требуется (подтверждено).
