...
## Частота показа кнопок (sampling)

Кнопки 👍/👎 под каждым ответом — навязчиво: визуальный шум на каждом
сообщении. Вместо этого показываем клавиатуру не всегда:

- всегда — на первом fast-reply ответе новому чату (первое впечатление,
  максимум сигнала при минимальной цене для UX);
- дальше — примерно раз в 8–12 ответов (случайный шаг в этом диапазоне на
  каждый чат, чтобы не выглядело как "по расписанию").

Реализация: счётчик `feedback_prompt_countdown` per-chat, в том же
in-memory `chat_data`, где уже живёт `web_enabled` (никакой новой durable
структуры, никакого нового privacy-риска — это счётчик, не контент).
При каждом fast-reply: если countdown <= 0 — показать кнопки и выставить
новый случайный countdown из диапазона 8–12; иначе — отправить ответ без
клавиатуры и декремент countdown на 1. Сбрасывается при рестарте процесса
(как и остальной ephemeral per-chat state) — это ожидаемо, не бага.

Это решение не меняет event-схему, `FeedbackStore` или privacy-модель —
только логику "показывать ли клавиатуру в конкретном ответе". Acceptance
tests должны покрыть: первый ответ нового чата всегда с клавиатурой; между
двумя показами клавиатуры проходит 8–12 ответов; countdown не хранит и не
логирует текст ответов.
...
1. Первый fast-reply ответ нового чата содержит inline-клавиатуру 👍/👎;
   дальше клавиатура появляется раз в 8–12 ответов (см. «Частота показа
   кнопок»), не на каждом сообщении.
...
- `ACTION_FEEDBACK` callback prefix + клавиатура на fast-reply ответах,
  показываемая по sampling-правилу (см. «Частота показа кнопок»), не на
  каждом ответе.
